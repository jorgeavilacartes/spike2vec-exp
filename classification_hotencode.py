...
logging.basicConfig(filename='logs.log', 
                    ...
                    level=logging.DEBUG,
                    filemode="w")
logging.info("clustering_hotencode.py")
...
with open(str(PATH_MODELS.joinpath("rff_hotencode.sav")),"rb") as fp: 
...
test_encoding = tuple(
                    encoder(padding(spike_from_fasta(path))) for path in paths_test
                    )
...
for classifier, clf in classifiers.items():
    ...
    joblib.dump(clf, str(PATH_MODELS.joinpath(f"{classifier}-hotencode.sav")))
    ...
    pd.DataFrame(list_metrics).to_csv(PATH_RESULTS.joinpath(f"test-{classifier}-hotencode.csv"))